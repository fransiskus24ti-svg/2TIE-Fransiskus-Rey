...
// Helper untuk menggantikan alpha (pakai rgba)
  ...
  return (
    <Zoom in timeout={300}>
      <Card sx={{ 
        ...
        background: `linear-gradient(135deg, ${rgba(color, 0.06)} 0%, ${rgba(color, 0.03)} 100%)`,
        transition: 'all 0.3s cubic-bezier(0.4, 0, 0.2, 1)',
        cursor: 'pointer',
        '&:hover': { 
          transform: 'translateY(-6px)', 
          boxShadow: `0 12px 28px ${rgba(color, 0.2)}`,
        ...
        '&::after': {
          ...
          right: 0,
          height: '4px',
          background: `linear-gradient(90deg, ${color} 0%, ${rgba(color, 0.5)} 100%)`,
        }
      }}>
        ...
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          ...
          <Chip
            ...
            sx={{
              ...
              fontSize: '0.7rem',
              fontWeight: 700,
              bgcolor: isTrendPositive ? rgba('#10b981', 0.12) : rgba('#ef4444', 0.12),
          ...
          notifications.map(notif => (
            <ListItem
              ...
              sx={{
                ...
                bgcolor: notif.read ? 'transparent' : rgba(theme.palette.primary.main, 0.05),
                cursor: 'pointer',
                '&:hover': { bgcolor: rgba(theme.palette.primary.main, 0.08) },
              }}
      ...
      <DialogContent sx={{ p: 3 }}>
        <Stack spacing={2}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            <Avatar sx={{ width: 56, height: 56, bgcolor: rgba(product.color, 0.1) }}>
              <InventoryTwoToneIcon sx={{ color: product.color }} />
            </Avatar>
            <Box>
              <Typography variant="h6" fontWeight={700}>{product.nama}</Typography>
              <Chip label={product.status} size="small" sx={{ bgcolor: rgba(product.color, 0.1), color: product.color }} />
            </Box>
          </Box>
          <Divider />
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
              ...
              <LinearProgress variant="determinate" value={product.progress} sx={{ height: 8, borderRadius: 4, mt: 0.5, bgcolor: rgba(product.color, 0.1), '& .MuiLinearProgress-bar': { bgcolor: product.color } }} />
            </Grid>
          </Grid>
        </Stack>
      </DialogContent>
        ...
        <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ xs: 'flex-start', sm: 'center' }} sx={{ mb: 4, gap: 2 }}>
          ...
          <Stack direction="row" spacing={1.5} alignItems="center" flexWrap="wrap">
            ...
            <Tooltip title="Refresh Data">
              <IconButton onClick={handleRefresh} sx={{ bgcolor: rgba('#3b82f6', 0.1) }}>
                <RefreshIcon sx={{ fontSize: '1.2rem' }} />
              </IconButton>
            </Tooltip>
            <Tooltip title="Export CSV">
              <IconButton onClick={handleExport} sx={{ bgcolor: rgba('#10b981', 0.1) }}>
                <GetAppIcon sx={{ fontSize: '1.2rem' }} />
              </IconButton>
            </Tooltip>
            <Tooltip title="Print Dashboard">
              <IconButton onClick={handlePrint} sx={{ bgcolor: rgba('#f59e0b', 0.1) }}>
                ...
                <Paper sx={{ p: 2.5, borderRadius: '20px', border: `1px solid ${rgba('#3b82f6', 0.15)}` }}>
                  <Stack direction="row" justifyContent="space-between" alignItems="center" mb={2}>
                    <Typography variant="h6" fontWeight={700}>
                      ...
                      <Chip label="Live" size="small" icon={<FiberManualRecordIcon sx={{ fontSize: '0.6rem', color: '#10b981 !important' }} />} sx={{ ml: 1, bgcolor: rgba('#10b981', 0.1), color: '#10b981', fontWeight: 600 }} />
                    </Typography>
                ...
                <Stack spacing={1.5}>
                  {salesActivity.map((act, idx) => (
                    <Grow in timeout={idx * 100} key={idx}>
                      <Paper sx={{ p: 1.5, borderRadius: '16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 1, transition: 'all 0.2s', '&:hover': { bgcolor: rgba(act.viaColor, 0.04) } }}>
